...
function main() {
  ...
  try {
    ...
    console.log(
      `[opcbrain] No config at ${configPath}; wrote a placeholder so the gateway can boot.`,
    );
    console.log('[opcbrain] Open the Web UI to finish onboarding (provider + API key).');
  } catch (error) {
    console.warn(
