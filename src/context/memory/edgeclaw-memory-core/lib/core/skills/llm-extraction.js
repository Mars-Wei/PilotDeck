...
function mergeClassificationLabels(labels, fallback) {
    if (!fallback || labels.some((label) => label.type === fallback.type))
        return labels;
...
function looksLikeExplicitRememberRequest(text) {
...
function looksLikePersonalUserFactText(text) {
    const normalized = normalizeWhitespace(stripExplicitRememberLead(text));
    if (!normalized || looksLikeCollaborationRuleText(normalized))
        return false;
    if (looksLikeConcreteProjectMemoryText(normalized) || looksLikeProjectFollowUpText(normalized))
        return false;
...
function inferLocalMemoryRoute(query) {
...
function buildSyntheticUserMemoryClassificationLabel(text) {
    if (!looksLikeExplicitRememberRequest(text) || !looksLikePersonalUserFactText(text))
        return null;
