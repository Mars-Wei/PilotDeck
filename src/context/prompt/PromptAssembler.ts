 ...
 * commands / skills) but uses OPC Brain-authored copy.
 *
...
export class PromptAssembler {
  ...
  private buildDefaultSystemPrompt(input: PromptAssemblerInput): string[] {
    const lines: string[] = [
      "You are OPC Brain, an AI agent runtime. You execute tasks across CLI, TUI, web, and chat channels by calling structured tools and reasoning over their results.",
  ...
  switch (mode) {
    ...
    case "plan":
      return [
        ...
        "4. Create or refine a markdown plan file under the project's `.opcbrain/plans/` directory",
        ...
        "- DO NOT call write_file, edit_file, create_file, or bash with any command that writes, deletes, or modifies files, except for markdown plan files under `.opcbrain/plans/`",
        "- You may use read-only tools freely, and you may write only markdown plan files under `.opcbrain/plans/` while plan mode is active",
