...
const RESERVED_OPCBRAIN_PATTERN =
...
export function validateMarketplaceName(name: string): string | undefined {
  ...
  if (NON_ASCII_PATTERN.test(name) || RESERVED_OPCBRAIN_PATTERN.test(name)) {
    return "Marketplace name impersonates an official OPC Brain marketplace.";
  }
  return undefined;
}
