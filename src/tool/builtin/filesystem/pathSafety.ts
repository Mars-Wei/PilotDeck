...
export function resolvePilotDeckWorkspacePath(
  ...
  if (!root) {
    return {
      ok: false,
      error: toolError("path_not_allowed", `Path ${inputPath} is outside the OPC Brain workspace.`),
    };
  }
  ...
  if (options?.mustExist) {
    ...
    if (!isPathWithinRoot(real, realRoot)) {
      return {
        ok: false,
        error: toolError("path_not_allowed", `Path ${inputPath} resolves outside the OPC Brain workspace.`),
      };
    }
  }
