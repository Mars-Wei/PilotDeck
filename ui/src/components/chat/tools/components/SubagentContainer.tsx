...
export const SubagentContainer: React.FC<SubagentContainerProps> = ({
  ...
  const description = parsedInput?.description || '正在运行任务';
  ...
  const title = `子智能体 / ${subagentType}: ${description}`;
      ...
      >
        ...
        {currentTool && !isComplete && (
          <div className="mt-1 flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span className="text-gray-400 dark:text-gray-500">当前：</span>
        ...
        {isComplete && isFailed && (
          <div className="mt-1 flex items-center gap-1.5 text-xs text-red-600 dark:text-red-400">
            ...
            <span>失败（{childTools.length} 个工具）</span>
          </div>
        )}
        ...
        {isComplete && !isFailed && (
          <div className="mt-1 flex items-center gap-1.5 text-xs text-green-600 dark:text-green-400">
            ...
            <span>已完成（{childTools.length} 个工具）</span>
          </div>
        )}
